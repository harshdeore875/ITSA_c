...
export default function FeaturedEventsSection() {
  ...
  return (
    <section ref={containerRef} className="py-20 bg-navy relative overflow-hidden">
      {/* Background decoration */}
      <div className="absolute bottom-0 right-0 w-96 h-96 bg-violet-500 rounded-full mix-blend-multiply filter blur-3xl opacity-5" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          ...
          <p className="text-gray-400 text-lg">Discover our most exciting upcoming events </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {featuredEvents && featuredEvents.length > 0 ? (
            ...
            <div
              key={event.id}
              className="event-card group rounded-2xl overflow-hidden bg-slate border border-indigo-900/30 hover:border-indigo-500/50 transition-all duration-300 hover-lift"
            >
              {/* Image */}
        ...
        <div className="text-center mt-12">
          <button className="px-8 py-3 border-2 border-indigo-500 text-indigo-400 rounded-lg hover:bg-indigo-500/10 transition-all duration-300 font-semibold">
            View All Events
          </button>
        </div>
      </div>
